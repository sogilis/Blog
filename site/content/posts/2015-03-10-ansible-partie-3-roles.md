...
Nous voyons avec cela comment utiliser les variables. Pour utiliser une variable, il existe la syntaxe `{{ *variable_name* }}`. Cette syntaxe correspond au langage de template [Jinja2](http://jinja.pocoo.org/) et nécessite d'inclure les variables entre quotes pour rester valide YAML. Voir la [documentation Ansible sur les variables](https://docs.ansible.com/ansible/latest/user_guide/playbooks_variables.html).
